...
public class Create_lead_activity extends AppCompatActivity {
                ...
                if(!firstNameEdit.getText().toString().isEmpty() && !lastNameEdit.getText().toString().isEmpty() &&
                    ...
                    client.newCall(request).enqueue(new Callback() {
                        ...
                        final String myreponce=response.body().string();
                            Create_lead_activity.this.runOnUiThread(new Runnable() {
                                @Override
                                public void run() {
                                    try {
                                        /*if (response.code() == 200) {
                                            Toast.makeText(Create_lead_activity.this, "Lead added successfully", Toast.LENGTH_SHORT).show();
                                            // startActivity(new Intent(getApplicationContext(), CorrespondenceActivity.class).putExtra("Company ID", CompanyID));
                                        } else {
                                            Toast.makeText(Create_lead_activity.this, "Error Occurred: ", Toast.LENGTH_SHORT).show();
                                        }*/
                                        Toast.makeText(Create_lead_activity.this, myreponce, Toast.LENGTH_SHORT).show();
                                    } catch (Exception e) {
                                        e.printStackTrace();
                                    }
                                }
